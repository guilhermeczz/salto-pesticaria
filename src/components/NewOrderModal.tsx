...
export function NewOrderModal({ open, onClose, editOrderId, initialCustomerName, initialCart, initialNotes }: NewOrderModalProps) {
  ...
  // 👇 ÂNCORA DE ROLAGEM: Para jogar a tela pro topo na Revisão
  ...
  // 👇 EFEITO DE ROLAGEM: Sempre que o 'showSummary' mudar, joga pro topo!
  useEffect(() => {
      ...
      <div className="w-full max-w-5xl h-full max-h-[90vh] bg-background rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-up border border-border/50">
        ...
        <div className="flex justify-between items-center px-6 py-5 border-b border-border/50 bg-card/50 backdrop-blur-sm z-10">
          <h2 className="text-2xl font-bold flex items-center gap-3 text-foreground tracking-tight">
            {showSummary && (
              <button onClick={() => setShowSummary(false)} className="mr-2 p-2 hover:bg-muted rounded-full transition-all hover:-translate-x-1">
            ...
            {editOrderId ? 'Editando Pedido' : 'Montar Novo Pedido'}
          </h2>
          <button onClick={onClose} className="p-2.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive rounded-xl transition-all hover:rotate-90">
            <X className="w-6 h-6" />
          </button>
        </div>

        {/* CONTEÚDO PRINCIPAL COM A ÂNCORA REF ADICIONADA */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-8 scroll-smooth custom-scrollbar">
          {showSummary ? (
            <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
              ...
              <div className="bg-card p-6 rounded-3xl border border-border/60 shadow-sm">
                <h3 className="font-semibold text-muted-foreground mb-4 uppercase tracking-wider text-sm">2. Cliente e Observações</h3>
                <p className="text-sm text-muted-foreground mb-1">Nome na comanda/mesa:</p>
                <p className="font-bold text-xl text-foreground bg-background p-4 rounded-xl border border-border/40">{customerName}</p>
                {notes && (
                  <div className="mt-4">
              ...
              <div className="bg-card p-6 rounded-3xl border border-border/60 shadow-sm">
                <h3 className="font-semibold text-muted-foreground mb-4 uppercase tracking-wider text-sm">3. Conferência de Itens</h3>
                <div className="space-y-2.5">
                  {cartItems.map((item, i) => (
                    <div key={i} className="flex justify-between items-center p-4 bg-background border border-border/40 rounded-2xl hover:border-primary/30 transition-colors">
                      <div className="flex items-center gap-3">
                        <span className="font-black text-primary bg-primary/10 px-2 py-1 rounded-lg min-w-[36px] text-center">{item.quantity}x</span>
                        <span className="font-semibold text-[15px]">{item.productName}</span>
                      </div>
                      <span className="font-medium text-muted-foreground">{formatMoney(item.quantity * item.unitPrice)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="animate-fade-in">
              {/* ÁREA DE BUSCA E NOME - Lado a Lado no PC */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-8 bg-card p-5 sm:p-6 rounded-3xl border border-border/60 shadow-sm">
                <div>
                  <label className="text-xs font-bold text-muted-foreground mb-2 block uppercase tracking-wider">Cliente ou Mesa</label>
                  <input
                    value={customerName}
                    onChange={e => setCustomerName(e.target.value)}
                    placeholder="Ex: Mesa 04 ou João"
                    /* 👇 FUNDO BRANCO E TEXTO PRETO FORÇADO 👇 */
                ...
                <div>
                  <label className="text-xs font-bold text-muted-foreground mb-2 block uppercase tracking-wider">Pesquisar Cardápio</label>
                  ...
                  <div key={catId} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                      {prods.map((p: any) => {
                        ...
                        return (
                          <div key={safeId} className="flex justify-between items-center p-4 bg-card border border-border/50 rounded-2xl transition-all duration-300 hover:border-primary/40 hover:shadow-md hover:-translate-y-0.5 group">
              ...
              <div className="mt-8 bg-card p-5 sm:p-6 rounded-3xl border border-border/60 shadow-sm">
                <label className="text-xs font-bold text-muted-foreground mb-3 block uppercase tracking-wider">Anotações para a Cozinha</label>
                <textarea
                  value={notes}
        ...
        <div className="p-4 sm:p-6 border-t border-border/50 bg-card/80 backdrop-blur-md z-10">
          {!showSummary ? (
            ...
            >
              <span>{loading ? 'Enviando para a Cozinha...' : 'Confirmar e Imprimir'}</span>
              <span className="bg-black/20 px-3 py-1.5 rounded-xl backdrop-blur-sm">{formatMoney(total)}</span>
            </button>
          )}
        </div>
